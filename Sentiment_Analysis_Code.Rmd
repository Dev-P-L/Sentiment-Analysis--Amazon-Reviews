...
In this project, **sentiment analysis** has been conducted on an Amazon sample provided in UCI Machine Learning Repository. 

88 % prediction accuracy has been reached on the validation set, against 50 % with a baseline model. Which factors have produced that improvement with 38 percentage points?
...
**Text mining** has brought additional accuracy improvement with 12.7 percentage points. Insights have been determinant: in decision trees predominate some tokens conveying subjective information; but other tokens containing subjective information are not used in false negatives and false positives. Such ignored subjective information has been retrieved from random samples of false negatives and false positives, exclusively on the training set; customized lists have been established with tokens sorted as having either positive or negative sentiment orientation; occurrences of these tokens in reviews have been replaced with either a positive or a negative generic token. Another insight has been about negation impact: negation has been fruitfully integrated. 

**Machine learning optimization** has been performed across 10 models and has boosted accuracy with 3.6 additional percentage points. Testing has been conducted on accuracy distributions across bootstrapped resamples. eXtreme Gradient Boosting has emerged as the most performing model in this project. 
...
It is comprised of twelve files. All code is included in Sentiment_Analysis.Rmd. It does not show in the final HTML report, called Sentiment_Analysis_Final_Report.html. 

For your convenience, the dataset has already been downloaded onto my GitHub repository wherefrom it will be automatically retrieved by the code from Sentiment_Analysis.Rmd. If you so wish, you can also easily retrieve the dataset from https://archive.ics.uci.edu/ml/datasets/Sentiment+Labelled+Sentences and adapt the Sentiment_Analysis.Rmd code accordingly.

You can knit Sentiment_Analysis.Rmd (please in HTML) and produce Sentiment_Analysis_Final_Report.html on your own computer. On my laptop, running Sentiment_Analysis.Rmd takes approximately six hours. For information, here are some characteristics of my work environment:

 - R version 3.5.1 (2018-07-02) -- "Feather Spray",
 - RStudio Version 1.1.456,
 - Windows 10.

Some packages are required in Sentiment_Analysis.Rmd. The code from Sentiment_Analysis.Rmd contains instructions to download these packages if they are not available yet. 
...
As explained on the UCI website, data is organized in a CSV file in two columns. In the first column, there are 1,000 Amazon product reviews (sentences). In the second column, there is a positive or negative evaluation; the ratio of positive evaluations is 50 %.
...
After being downloaded, data will be transposed into a corpus. Then it will be processed in NLP: words will be lowercased, punctuation marks will be removed as well as stopwords and finally words will be stemmed. 
...
There are topic-related tokens such as "phone", tokens conveying subjective information such as "great", etc. Before analyzing token categories, let's check up the adequacy of results. 
...
stopwords with variants such as "dont", "couldnt". This will be done.
...
This is another scenario: "don't" has been written in a standard way, but all punctuation marks have been removed, consequently it has become "dont" and is no longer identical to the stopword "don't" and, very logically, has not been removed. Consequently, stopwords containing apostrophes should be removed before removing punctuation. Or punctuation marks should be removed except for apostrophes and hyphens: this solution will be applied because it meets the objective and allows to immediately get rid of all other marks of punctuation. 

After examining all tokens coming from training reviews, other oddities have been discovered. There are several unigrams that seem to originate from bigrams, e.g. "brokeni" at row 24.
...
In further text mining and machine learning steps, "brokeni" would be treated differently than "broken", which can be seen on the same row, i.e. on row 24 of the tokens from training reviews. This has to be changed. 
...
Instead of the function removePunctuation() from the package tm, specific for loops will be developed, preprocessing reviews according to the needs stated above: 

- punctuation marks other than apostrophes will be replaced with white space characters instead of just being removed;
- short forms will then be removed and afterwards remaining apostrophes will also be replaced with white space characters.

Moreover, among stopwords, short forms (contractions) need to be specifically treated. Additional needs of breakdown might also emerge. Consequently, stopwords will be split. Starting from the list delivered by the function stopwords("english") from the package tm, four CSV files will be produced.
...
As well as many other oddities. I leave uncorrected some spelling errors, such as "disapoint" or "dissapoint", because this is no repetitive structure and occurrence seems marginal. 
...
By default, the train() function would train across 3 values of cp (the complexity parameter) and 25 bootstrapped resamples for each tuned value of cp. 
...
As far as the number of tuned values is concerned, let's upgrade it to 15 to increase the odds of improving accuracy, especially as rpart runs rather quickly. Will accuracy improve?
...
Accuracy increases somewhat. For the record, let's have a look at a graph showing how accuracy evolves across the 15 cp values chosen by the train() function. 
...
On the whole training set, the rpart model without tuning delivers 78 % accuracy and the rpart model with tuning gives 79 %. Both levels are substantially higher than accuracy provided by the baseline model. The baseline model would predict a positive evaluation for all reviews (or alternatively a negative evaluation for all reviews) since prevlence is 50 %. What accuracy level would the baseline model deliver? 
...
Although the majority of tokens are conveying subjective information in the decision tree, many tokens with subjective information are present among false negatives but do not show in the decision tree. It is probably a matter of word (or token) frequency. This can be first checked up in the wordcloud that has already been visualized. 
...
In conclusion, it might be impactful to garner subjective information conveyed by tokens such as "super", "faster", "prettier", "infatu", "awesom", etc. Since CART doesn't do it, why not replace such tokens with a generic token either positive or negative? This would impower subjective information by building high frequency generic tokens only typified by sentiment orientation.  

In this project, polarity of some tokens conveying subjective information will be inserted in additional files. That is one avenue of improvement.
...
Let's go back to the table above that is comprised of 12 reviews corresponding to false negatives in the model cart_tuned. More particularly, let's retrieve the review with a negational unigram. 
...
On the list, the first stemmed unigram associated with "not" is "impress". Let's localize the reviews producing "impress" to get an idea of frequency and of decisiveness about polarity. 
...
Up to now, negation has been shown in negational unigrams but negation can also be encapsulated into short forms (also called "contractions"). This sample of false negatives, which has been selected at random, is not comprised of negative short forms. But the same procedure, applied to false positives, delivers several of them. 
...
In six cases, there is negation, which machine learning could not take into account since these negation forms had been removed through NLP. There are four negative short forms. 

This shows the importance of negation, be it under the form of negational unigrams ("no", etc.) or under the form of negational short forms.

For the record, in the bale of false positives, blue background has been used where at least one token per review expresses sentiment orientation, from a human point of view: "difficult" or "sorry". This again shows the interest of replacing ignored subjective information tokens with one generic tokens expressing either negative (here) or positive sentiment. 

Green background has been used in simple negation scenarios: there is negation and at least one unigram conveying positive subjective information. 

Gray background has been used for more complex cases, with e.g. sarcasm: these cases will be analyzed in the next section.
...
Sentiment can also be expressed through associations of words beyond cases of negation already treated. 

In some cases, these are rather stereotyped phrases. False negatives have provided two examples: "knows what they're doing" (competency statement) or "as described" (compliance statement). Some of these phrases will be listed among positive multigrams and replaced with a generic token with positive orientation. 
...
False positives have delivered some difficult cases: reviews with figurative wording, sarcasm, irony, metaphors, multifaceted reviews, etc. 
...
Actually, the impact ranking of predictors is rather different than the ranking in the CART decision tree: e.g. "phone" is ranked in the 11th position. In the first 16 positions, there are three topic-related tokens ("price", "phone" and "product") while there is none in the first 16 positions in the decision tree. In a snapshot, three topic-related tokens get some importance in  Random Forest and Random Forest performs better: that is a reason to keep topic-related tokens in the bag of words. Especially so since other models than CART will be trained in the machine learning section. 
...
There remain false negatives and false positives with the rf model. Let's have a look at the false negatives, which are more numerous. Here is a sample of the false negatives from Random Forest. 
...
- by comparing token frequency between bag of words (wordcloud and histogram) and decision tree
- and by perusing false negatives and positives and pinpointing ignored but hopefully usable subjective information.
...
## VI. INFORMATION RETRIEVAL USING INSIGHTS
...
First, negative short forms will be introduced into the bag of words and impact on accuracy will be tested. 
...
Adding negative short forms such as "isn't" does not impact accuracy level. Consequently, another try will be done about negative short forms: instead of being added, negative short forms will be replaced with "not". 
...
In the line of these insights, these n-grams have been listed and classified as positive and negative. They have been stopped in four files and uploaded in the GitHub repository https://github.com/Dev-P-L/Sentiment-Analysis :
...
Efficacy-minded rules will be applied in NLP.

First, the polarized n-grams will be preceded and followed by one white space character when looking for occurrences in reviews. Otherwise, in the bag of words, the n-gram "most inconvi" would become "most in subjpo " (because "convi" is a polarized unigram in subj_pos_unigrams.csv) and then "  subjpo " (because "most" and "in" are stopwords in stopwords_without_apostrophe.csv)! A negatively oriented multigram would become a positively oriented unigram! Consequently, one white space character is added in front of and at the end of each polarized n-gram before looking for matching occurrences in NLP-transformed reviews, in order to avoid replacing substrings.
...
Polarizing n-grams that convey subjective information has dramatically boosted accuracy from 81 % to 92 %. Nevertheless, let us stay realistic: text mining has been performed on the training set and is not necessarily, or at least not necessarily entirely, transposable to the validation set. 

To get some visual representation of changes, let's have a look at the wordcloud. 
...
As shown in the last two rows, thanks to polarization, accuracy has jumped from 81 % up to 92 %, which is impressive. 

More impressive: sensitivity has sprung from 66 % to 89 %. This is linked to false negatives management. Fasle negatives have been a recurrent weak point in machine learning results up to now. But special attention has been paid to them in debriefing previous machine learning results, in perusing false positives and much more false negatives and in canalizing n-grams with subjective information towards either a postive generic token or a negative generic token. 
...
At the previous step, results were less good, especially for false negatives, as already shown in a table. The number of false negatives has been crushed from 114 to 38; parallelwise, the number of true negatives has climbed from 220 to 296. There is also a decrease in false positives, much more modest though, from 22 to 17. 

After improving accuracy thanks to NLP and text mining, new accuracy improvements will be looked for in the next section through machine learning, diversifying models.
...
In order to predict on the validation set, the training set and the validation set have to contain the same columns, i.e. same labels (dependent variable) and the same predictors. 
...
The validation will be constructed in the same way as the training set, but independently: NLP, negational unigrams and polarization. Columns that are in the validation set but not in the training will be removed. Columns that are in the training set but not in the validation set will be added to the validation set as null vectors. 
...
Instead of using existing dictionaries about sentiment orientation, customized lists have been built. Replacing tokens with two generic tokens according to sentiment polarity has provided an accuracy improvement with 10 percentage points on the validation set. This improvement has shown resilience since it has hardly shrunk from the training set, where it was 11 percentage points, to the 10 percentage points on the validation set, although list building has been operated exclusively on the training set. 
