...
# After the present chunk, there are CSS lines of code to further regulate layout:
# - no bullets in TOC,
# - no link repetition in PDF conversion,
# - double text justification,
# - color and font in title, headers, links and URLs,
# - body text font.
...
```{r Setting up work environment, include = FALSE}
# Summary : cleaning up user interface, installing and loading packages
...
# By the way, the leading white space character in " Pos" cares for " Pos" coming first in the confusion matrix. 
...
df <- reviews[ind_train, ] %>% 
  as.data.frame() %>% 
  `rownames<-`(ind_train) %>% 
  mutate(ro = rownames(.))

# Some simple statistics in a table: number of positives reviews and of negative ones. 

tab <- table(df$sentiment) %>%
  as.data.frame() %>%
...
knitr::kable(tab, "html", align = "c") %>% 
  ...
  column_spec(1, width = "2.5in", bold = T, color = "#08457E", 
              background = "#9bc4e2") %>%
  column_spec(2, width = "3in", bold = T, color = "#08457E", 
              background = "#9bc4e2")
...
Let's have a quick look at a few training reviews.

<br>

```{r A few examples of reviews from the training set}
# Extracting a 12-row sample from the training set
# in order to create a presentation table. 
set.seed(1)
seq <- createDataPartition(y = df$sentiment, times = 1, 
                           p = 12/length(ind_train), list = FALSE)

sampl <- df[seq, ] %>% as.data.frame() %>% 
  select(ro, text, sentiment) %>% 
  `rownames<-`(NULL)

# Building up presentation table.
tab <- sampl %>% as.data.frame() %>% 
  `colnames<-`(c("", "ILLUSTRATIVE SAMPLE OF TRAINING REVIEWS", 
                 "SENTIMENT"))
...
knitr::kable(tab, "html", align = "c") %>% 
  ...
  column_spec(2:3, bold = T, color = "#08457E", 
              background = "#9bc4e2")
rm(df, seq, sampl, tab)
```
...
# First, let's build up a data frame with only the 50 most frequent tokens from "sentSparse", the Document Term Matrix pruned by the sparsity process. 
...
wordcloud2(df, shape = 'square', color = 'random-light',
           backgroundColor = "#507786", shuffle = FALSE)
```
...
# Which rows from training reviews have a number greater than 1 
# in the column "dont"? This would mean that the corresponding 
# NLP-transformed reviews contain " dont ".
bin <- which(sentSparse$dont >= 1)

# What is the frequency of occurrence of " dont "?
df <- data.frame(length(bin)) %>% 
  `colnames<-`('# REVIEWS CONTAINING "dont"') %>%
...
```{r First scenario for "dont"}
df <- data.frame(reviews_train$text[bin[17]]) %>%
  `colnames<-`('REVIEW GENERATING "dont" IN BAG OF WORDS') %>%
...
```{r Second scenario for "dont"}
df <- data.frame(reviews_train$text[bin[19]]) %>% 
  `colnames<-`('REVIEW GENERATING "dont" IN BAG OF WORDS') %>%
  `rownames<-`("Set of Training Reviews")
knitr::kable(df, "html", align = "c") %>% 
  kable_styling(bootstrap_options = "bordered", 
...
knitr::kable(tokens[24, ], "html", align = "c") %>% 
  kable_styling(bootstrap_options = "bordered", 
                full_width = F, font_size = 16) %>%
  column_spec(1, bold = T, color = "#808080") %>%
  column_spec(2:6, bold = T, color = "#08457E", 
              background = "#9bc4e2") 
```
...
# We have to work on all tokens, upstream of the sparsity process, which the token "brokeni" couldn't pass since there is only one instance of "brokeni"! The corpus meets this requirement: it contains all tokens. 

# First, locating the review the token "brokeni" originates from.
...
set.seed(1)
wordcloud2(df, shape = 'forward', color = 'random-dark',
...
df <- data.frame(sprintf("%.4f", 
                  round(fit_cart_tuned_av0$bestTune, 4))) %>%
...
knitr::kable(df, "html", align = "c") %>% 
      kable_styling(bootstrap_options = "bordered", 
                full_width = F, font_size = 16) %>%
      column_spec(1, bold = T, color = "#808080") %>%
      column_spec(2, bold = T, color = "#08457E", 
                  background = "#9bc4e2")
...
knitr::kable(tab_av0, "html", align = "c") %>% 
  kable_styling(bootstrap_options = "bordered", 
                full_width = F, font_size = 16) %>%
  row_spec(1, bold = T, strikeout = T, color = "#08457E", 
           background = "#9bc4e2") %>%
  row_spec(2, bold = T, color = "#08457E", 
           background = "#9bc4e2") %>%
  row_spec(3, bold = T, color = "#205030", 
           background = "#a7e3bb")
...
Let's now have a look at false negatives, false positives and topic-related tokens. 
...
# To identify false negatives, we need both the review polarity and the predicted review polarity. Consequently, we combine both variables in one data frame.  
df <- data.frame(sentiment = reviews_train$sentiment,
                 pred = fitted_cart_tuned_av0) 

# We have a false negative if review polarity is positive and if predicted review polarity is negative. If CART delivers a false negative for a specific row, then the first command below produces 1; if it is false positive, the result is -1; a true positive or a true negative gives 0.
FN_train <- ifelse(df$sentiment == " Pos", 1, 0) - 
            ifelse(df$pred == " Pos", 1, 0)

# If the first command gives 1 (false negative), then the second command delivers 1 as well; otherwise, the second command delivers 0. Consequently, false negatives are identified by a 1 in FN_train after the second command. 
FN_train <- ifelse(FN_train == 1, 1, 0)

# Row numbers corresponding to the 98 false negatives.
FN <- which(FN_train == 1)

# Now let's build up an interactive table with all false negatives delivered by CART with tuning, i.e. the 98 false negatives. 

# Garnering data.
df <- data.frame(row_train = FN,
                 review = as.character(1:length(FN)),
                 tokenized = as.character(1:length(FN)))

for (i in 1:length(FN)) {
  row <- FN[i]
  df[i, 2] <- reviews_train$text[row]
  df[i, 3] <- corpus_av0[[row]]$content
}

rm(i, row)

# Creating interactive data table.
# The package DT is used. 
...
In the interactive table above, if we scroll through false negatives several scenarios appear. Let's classify false negatives into four scenarios, identified by the pivotal pieces of information that that were not used by CART to produce the right polarity, i.e. the positive polarity:

- subjective information unigrams not used; 
...
#### 2. First Scenario: Subjective Information Unigrams Unused

When consulting the table of false negatives above, we can pinpoint some words/tokens encompassing some subjective information that points to the right polarity. In spite of these words/tokens, the polarity has been wrongly read by CART. Why?
...
subjective information that is clear from a human point of view. Let's just pinpoint "super", "fast", "faster", "prettier", "infatuated", "wise", "awesome", "rocks", etc., or, after stemming "super", "fast", "faster", "prettier", "infatu", "wise", "awesom", "rock", etc. 
...
The difference between the two categories can sometimes be tricky and both categories will be referred to altogether in this project using phrases such as "subjective information" or "tokens conveying subjective information". 
...
# Removing the last column from sentSparse_av0, the data frame originating from the Document Term Matrix downstream of the sparsity process. 

df <- sentSparse_av0[, - ncol(sentSparse_av0)]

# Building up a data frame with the tokens and the frequency of occurrence and keeping only the top 30 tokens in frequency. 
...
# Centering the interactive graph and centering it.
htmltools::div(p, align = "center" )
...
# From the 12 false negatives generated by CART and picked up at random, retrieving the one with a "negational unigram" (a single word expressing negation, possibly in a short form).

tab <- df_FN_cart[5, ] %>% select(- 1)

# Creating a presentation table and printing it.
