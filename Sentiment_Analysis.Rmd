...
Sentiment analysis has been conducted on an Amazon data sample provided by UCI Machine Learning Repository. In that sample, 1,000 reviews (sentences) had received sentiment orientation, i.e. " Pos" or "Neg". 

The challenge is to maximize accuracy in predicting sentiment on a 1-third validation set. Accuracy metric has been deemed representative since prevalence of positive class is 50 %.

An accuracy level of 88 % has been reached on the validation set at the end of a three-tier analysis.

First, **Natural Language Processing** has been conducted: lowercasing, punctuation handling, negation handling, stopword removal, stemming, tokenization and bag of words. 

Second, **text mining** has focused on word frequency, wordclouds, decision trees and analysis of false positives and false negatives, which have been the weak point. Perusing random samples of false negatives and false positives has opened up two avenues for improvement: integrating negational unigrams ("not", etc.) and establishing lists of positive n-grams and negative n-grams from subjective information n-grams that were present in false negatives or positives but didn't show in decision trees. In reviews, all  occurrences matching these lists have been replaced with either a generic positive token or a generic negative token. Alternative grammar has been taken on board. Text mining has been guided by CART being used as a performance yardstick.  

Third, **machine learning** has boosted accuracy with 10 models. Testing has been conducted on accuracy distributions across bootstrapped resamples. eXtreme Gradient Boosting has emerged. 

Altogether, NLP, text mining and machine learning have uplifted accuracy on the validation set from 50 % with the baseline model to 88 % with xgbLinear. 
...
tab <- data.frame(matrix(tab, ncol = 2, nrow = 2, byrow = FALSE)) %>%
  ...
  `rownames<-`(c("Predicted positive", "Predicted negative"))
knitr::kable(tab, "html", align = "c") %>% 
  kable_styling(bootstrap_options = "bordered", 
                full_width = F, font_size = 16) %>%
...
colnames(df) <- c('TRAINING REWIEWS CONTAINING "impressed"')
...
In the three training reviews on #ede1f5 background, negation unigrams completely change polarity of review. Among the training reviews, there are three reviews containing "not impressed". Moreover, in the second of these reviews, there is also "not recommend" and "not laughing". This illustrates the usefulness of including negational words in one way or another. 
...
kable(df, "html", align = "c") %>% 
  ...
  row_spec(c(1, 5, 9, 11, 12), bold = T, color = "#205030", 
           background = "#a7e3bb") %>%
  row_spec(c(2, 3, 4, 6:7), bold = T, color = "#333333", background = "#d8d8d8") 
...
knitr::kable(tab, "html", align = "c") %>% 
  kable_styling(bootstrap_options = "bordered", 
                full_width = F, font_size = 16) %>%
  column_spec(1, bold = T, color = "#08457E") %>%
  column_spec(2, bold = T, color = "#333333", background = "#d8d8d8")
rm(tab)
```
