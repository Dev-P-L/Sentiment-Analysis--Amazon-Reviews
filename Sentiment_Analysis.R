...
# The rpart/train/cv model delivers an accuracy level of
...
df <- data.frame(fit_cart_tuned_av0$bestTune) %>%
  `rownames<-`("CART with Tuning on cp") %>% `colnames<-`("Final Model's cp")
knitr::kable(df, "pandoc")
rm(df)
...
# Accuracy is 78-79 % with models rpart and rpart/train/cv, 
...
accuracies <- c(
  ...
  cm_cart_av0$overall["Accuracy"],
  cm_cart_tuned_av0$overall["Accuracy"])
sensitivities <- c(
  cm_baseline$byClass["Sensitivity"],
  cm_cart_av0$byClass["Sensitivity"],
  cm_cart_tuned_av0$byClass["Sensitivity"])  
specificities <- c(
  cm_baseline$byClass["Specificity"],
  cm_cart_av0$byClass["Specificity"],
  cm_cart_tuned_av0$byClass["Specificity"])  
posPredValues <- c(
  cm_baseline$byClass["Pos Pred Value"],
  cm_cart_av0$byClass["Pos Pred Value"],
  cm_cart_tuned_av0$byClass["Pos Pred Value"])  
negPredValues <- c(
  0,
  cm_cart_av0$byClass["Neg Pred Value"],
  cm_cart_tuned_av0$byClass["Neg Pred Value"])

colname <- c("MODEL", "SHORT DESCRIPTION", "ACCURACY", "SENSITIVITY", 
             "NEG PRED VAL", "SPECIFICITY", "POS PRED VAL")
tab_av0 <- data.frame(models, 
                      description, 
                      round(accuracies, 4),
...
rm(colname, models, description, accuracies, sensitivities, negPredValues,
   specificities, posPredValues)
rm(cm_baseline, pred_baseline)

# Let's further comment. 
...
# Getting a ranking of predictor importance. 
...
colname <- c("REVIEW", "TOKENIZATION", "USABLE SUBJECTIVE INFORMATION")
comment <- c("super", "fast + faster", "prettier + sharp", "infatu", "wise",
             "awesom", "rock", "troubl", "?", "?", "secur", "quick")
df <- df %>% mutate(com = comment) %>% `colnames<-`(colname)
rm(colname, comment)
...
# emanating from the final CART model.
df_FN_cart
rm(df_FN_cart)
...
description <- c("negation short forms + rpart + cp tuning", 
                  "negation unigrams + rpart + cp tuning")
accuracies <- c(
  cm_cart_tuned_av1_a$overall["Accuracy"],
  cm_cart_tuned_av1_b$overall["Accuracy"])
sensitivities <- c(
  cm_cart_tuned_av1_a$byClass["Sensitivity"],
  cm_cart_tuned_av1_b$byClass["Sensitivity"])  
specificities <- c(
  cm_cart_tuned_av1_a$byClass["Specificity"],
  cm_cart_tuned_av1_b$byClass["Specificity"])  
posPredValues <- c(
  cm_cart_tuned_av1_a$byClass["Pos Pred Value"],
  cm_cart_tuned_av1_b$byClass["Pos Pred Value"])  
negPredValues <- c(
  cm_cart_tuned_av1_a$byClass["Neg Pred Value"],
  cm_cart_tuned_av1_b$byClass["Neg Pred Value"])

colname <- c("MODEL", "SHORT DESCRIPTION", "ACCURACY", "SENSITIVITY", 
             "NEG PRED VAL", "SPECIFICITY", "POS PRED VAL")
tab_av1 <- data.frame(models, 
                      description, 
                      round(accuracies, 4),
                      round(sensitivities, 4),
                      round(negPredValues, 4),
...
knitr::kable(tab_av_0_1, "html", align = "c") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, font_size = 16) %>%
  row_spec(1:4, bold = T, color = "white", background = "blue") %>%
  row_spec(5, bold = T, color = "white", background = "green")

rm(cm_cart_tuned_av1_a, cm_cart_tuned_av1_b)
rm(models, description, colname, accuracies, sensitivities, negPredValues,
   specificities, posPredValues)
rm(tab_av0, tab_av1)
...
# This polarization will then be transfered into NLP-transformed reviews. 
# In reviews, all occurences of the positively
# polarized n-grams will be replaced with " subjpo ". 

# Analogously, in reviews, negatively polarized n-grams 
# will be replaced with " subjneg ". 
...
# to get apostrophes if first or last position if they are at 
# the beginning or the end of a review.
for (i in 1:nrow(reviews_train)) {
...
# with repetition (if case there are several of them).
for (i in 1:nrow(reviews_train)) {
  for (j in 1:12) {
...
# from subj_neg_multigrams.csv or subj_pos_multigrams.csv and,
# if it's the case, replacing with generic polarized unigram 
# " subjpo " or " subneg ".
for (i in 1:nrow(reviews_train)) {
...
# might have prevented taking some strings into account).
for (i in 1:nrow(reviews_train)) {
  for (j in 1:length(subj_neg_multigrams)) {
    corpus_av2[[i]]$content <- gsub(subj_neg_multigrams[j], " subjneg ", 
                                    corpus_av2[[i]]$content)
...
set.seed(1)
fit_cart_cv_av2 <- train(sentiment ~ .,
                      method = "rpart",
                      data = sentSparse_av2,
                      tuneLength = 15, 
                      metric = "Accuracy")
fitted_cart_cv_av2 <- predict(fit_cart_av2)
cm_cart_cv_av2 <- 
  confusionMatrix(as.factor(fitted_cart_cv_av2), 
                  as.factor(sentSparse_av2$sentiment))

# Result table
models <- c("cart_cv_av2")
description <- c("negational stopwords + polarization + rpart + tuning")
accuracies <- cm_cart_cv_av2$overall["Accuracy"]
sensitivities <- cm_cart_cv_av2$byClass["Sensitivity"]
specificities <- cm_cart_cv_av2$byClass["Specificity"] 
posPredValues <- cm_cart_cv_av2$byClass["Pos Pred Value"]  
negPredValues <- cm_cart_cv_av2$byClass["Neg Pred Value"]
...
knitr::kable(tab_av_0_1_2, "html", align = "c") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, font_size = 16) %>%
  row_spec(1:5, bold = T, color = "white", background = "blue") %>%
...
tab <- table(fitted_cart_cv_av2, sentSparse_av2$sentiment) %>% as.vector()
tab <- data.frame(matrix(tab, ncol = 2, nrow = 2, byrow = FALSE)) %>%
  `colnames<-`(c("Appreciating (ref)", "Criticizing (ref)")) %>%
  `rownames<-`(c("Appreciating (pred)", "Criticizing (pred)"))
knitr::kable(tab, "pandoc")

# At the previous step, results in less good, especially in false negatives.
tab <- table(fitted_cart_tuned_av1_b, sentSparse_av1_b$sentiment) %>% as.vector()
tab <- data.frame(matrix(tab, ncol = 2, nrow = 2, byrow = FALSE)) %>%
  `colnames<-`(c("Appreciating (ref)", "Criticizing (ref)")) %>%
  `rownames<-`(c("Appreciating (pred)", "Criticizing (pred)"))
...
# The number of false negatives has been crushed from 114 to 38; parallelwise,
...
#` This is linked to a completely different bag of words in frequencies.
set.seed(1)
wordcloud(colnames(sentSparse_av2), colSums(sentSparse_av2), min.freq = 10, 
          max.words = 50, random.order = FALSE, rot.per = 1/3, 
          colors = brewer.pal(8, "Dark2"), scale = c(4,.5))
...
prp(fit_cart_cv_av2$finalModel, uniform = TRUE, cex = 0.6, 
    box.palette = "auto")
...
# the training set has to be split into a smaller training test
# and a small test set. The purpose is to pick up the 
# best performing models without using the validation set. 

temp <- sentSparse_av2 %>% 
  mutate(row_names = as.integer(rownames(sentSparse_av2))) %>%
  as.data.frame()
set.seed(1)
ind <- createDataPartition(y = temp$sentiment, 
                           times = 1, p = 2/3, list = FALSE)
ind <- as.integer(ind)
train_small <- sentSparse_av2[ind, ]
test <- sentSparse_av2[-ind, ]

# CART
fit_cart <- train(sentiment ~ ., method = "rpart", data = train_small) 
fitted_cart <- predict(fit_cart)
cm_small_cart <- 
  confusionMatrix(as.factor(fitted_cart), as.factor(train_small$sentiment))

# Making predictions.
pred_cart <- predict(fit_cart, newdata = test)
cm_test_cart <- 
  confusionMatrix(as.factor(pred_cart), as.factor(test$sentiment))

# CART + CV
trControl = trainControl(method = "cv", number = 10, p = .9)
fit_cart_cv <- train(sentiment ~ .,
                    method = "rpart",
                    data = train_small,
                    tuneLength = 15, 
                    metric = "Accuracy",
                    trControl = trControl) 
fitted_cart_cv <- predict(fit_cart_cv)
cm_small_cart_cv <- 
  confusionMatrix(as.factor(fitted_cart_cv), as.factor(train_small$sentiment))

# Making predictions.
pred_cart_cv <- predict(fit_cart_cv, newdata = test)
cm_test_cart_cv <- 
  confusionMatrix(as.factor(pred_cart_cv), as.factor(test$sentiment))

# SVM

fit_svm <- train(sentiment ~ ., method = "svmRadialCost", data = train_small) 
fitted_svm <- predict(fit_svm)
cm_small_svm <-
  confusionMatrix(as.factor(fitted_svm), as.factor(train_small$sentiment))
...
cm_small_svm_cv <-
  confusionMatrix(as.factor(fitted_svm_cv), as.factor(train_small$sentiment))
...
fit_adaboost <- train(sentiment ~ ., method = "adaboost", data = train_small) 
fitted_adaboost <- predict(fit_adaboost)
cm_small_ada <-
  confusionMatrix(as.factor(fitted_adaboost), as.factor(train_small$sentiment))
...
fit_rf <- train(sentiment ~ ., method = "rf", data = train_small) 
fitted_rf <- predict(fit_rf)
cm_small_rf <-
  confusionMatrix(as.factor(fitted_rf), as.factor(train_small$sentiment))
...
fit_gbm <- train(sentiment ~ ., method = "gbm", data = train_small) 
fitted_gbm <- predict(fit_gbm)
cm_small_gbm <-
  confusionMatrix(as.factor(fitted_gbm), as.factor(train_small$sentiment))
...
cm_small_gbm_cv <-
  confusionMatrix(as.factor(fitted_gbm_cv), as.factor(train_small$sentiment))
...
fit_xgb <- train(sentiment ~ ., method = "xgbLinear", data = train_small) 
fitted_xgb <- predict(fit_xgb)
cm_small_xgb <-
  confusionMatrix(as.factor(fitted_xgb), as.factor(train_small$sentiment))
...
fit_monmlp <- train(sentiment ~ ., method = "monmlp", data = train_small) 
fitted_monmlp <- predict(fit_monmlp)
cm_small_mlp <-
  confusionMatrix(as.factor(fitted_monmlp), as.factor(train_small$sentiment))
...
# On the small training set and on the test set

models <- c("rpart", "rpart + cp tuning", "smvRadialCost", "svmRadialCost + cv",
            "adaboost", "rf", "gbm", "gbm + cv", "xgbLinear", "monmlp")
acc_small <- c(cm_small_cart$overall["Accuracy"],
               cm_small_cart_cv$overall["Accuracy"],
               cm_small_svm$overall["Accuracy"],
               cm_small_svm_cv$overall["Accuracy"],
               cm_small_ada$overall["Accuracy"],
               cm_small_rf$overall["Accuracy"],
               cm_small_gbm$overall["Accuracy"],
               cm_small_gbm_cv$overall["Accuracy"],
               cm_small_xgb$overall["Accuracy"],
               cm_small_mlp$overall["Accuracy"])
acc_test <- c(cm_test_cart$overall["Accuracy"],
              cm_test_cart_cv$overall["Accuracy"],
              cm_test_svm$overall["Accuracy"],
              cm_test_svm_cv$overall["Accuracy"],
              cm_test_ada$overall["Accuracy"],
              cm_test_rf$overall["Accuracy"],
              cm_test_gbm$overall["Accuracy"],
              cm_test_gbm_cv$overall["Accuracy"],
              cm_test_xgb$overall["Accuracy"],
              cm_test_mlp$overall["Accuracy"])
tab_test <- data.frame(Model = models, 
                  Accuracy_on_Small_Training_Set =  round(acc_small, 4),
                  Accuracy_on_Test_Set = round(acc_test, 4)) %>%
  arrange(- Accuracy_on_Test_Set)
kable(tab_test, "html", align = "c") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, font_size = 16) %>%
  row_spec(1:5, bold = T, color = "white", background = "green")

# The 5 bestperforming models out of 10 have been highlighted in green.
# I am tempted to build up an ensemble solution based on the 5 bestperforming
# models. But I am not really tempted by taking svmRadialCost and 
# svmRadialCost + cv since cross-validation impedes a little bit accuracy 
# on svmRadialCost; by the way, the same holds for gbm but not for rpart!

# Consequently, I am going to skip svmRadialCost + cv and take the 6th model
# on board instead of svmRadialCost + cv. Let's adapt the colored table.

kable(tab_test, "html", align = "c") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, font_size = 16) %>%
  row_spec(1, bold = T, color = "white", background = "green") %>%
  row_spec(2, bold = T, color = "blue", background = "white") %>%
  row_spec(3:4, bold = T, color = "white", background = "green") %>%
  row_spec(5:10, bold = T, color = "white", background = "blue")

# But I still wish to have a look at other performance metrics, i.e. 
# sensitivity, negative predictive value, specificity and 
# positive predictive value, on the test set.

models <- c("rpart", "rpart + cp tuning", "smvRadialCost", "svmRadialCost + cv",
            "adaboost", "rf", "gbm", "gbm + cv", "xgbLinear", "monmlp")
acc <- c(cm_test_cart$overall["Accuracy"],
              cm_test_cart_cv$overall["Accuracy"],
              cm_test_svm$overall["Accuracy"],
              cm_test_svm_cv$overall["Accuracy"],
              cm_test_ada$overall["Accuracy"],
              ...
              cm_test_xgb$overall["Accuracy"],
              cm_test_mlp$overall["Accuracy"])
sens <- c(cm_test_cart$byClass["Sensitivity"],
              cm_test_cart_cv$byClass["Sensitivity"],
              cm_test_svm$byClass["Sensitivity"],
              cm_test_svm_cv$byClass["Sensitivity"],
              cm_test_ada$byClass["Sensitivity"],
              cm_test_rf$byClass["Sensitivity"],
              cm_test_gbm$byClass["Sensitivity"],
              cm_test_gbm_cv$byClass["Sensitivity"],
              cm_test_xgb$byClass["Sensitivity"],
              cm_test_mlp$byClass["Sensitivity"])
neg <- c(cm_test_cart$byClass["Neg Pred Value"],
          cm_test_cart_cv$byClass["Neg Pred Value"],
          cm_test_svm$byClass["Neg Pred Value"],
          cm_test_svm_cv$byClass["Neg Pred Value"],
          cm_test_ada$byClass["Neg Pred Value"],
          cm_test_rf$byClass["Neg Pred Value"],
          cm_test_gbm$byClass["Neg Pred Value"],
          cm_test_gbm_cv$byClass["Neg Pred Value"],
          cm_test_xgb$byClass["Neg Pred Value"],
          cm_test_mlp$byClass["Neg Pred Value"])
spec <- c(cm_test_cart$byClass["Specificity"],
         cm_test_cart_cv$byClass["Specificity"],
         cm_test_svm$byClass["Specificity"],
         cm_test_svm_cv$byClass["Specificity"],
         cm_test_ada$byClass["Specificity"],
         cm_test_rf$byClass["Specificity"],
         cm_test_gbm$byClass["Specificity"],
         cm_test_gbm_cv$byClass["Specificity"],
         cm_test_xgb$byClass["Specificity"],
         cm_test_mlp$byClass["Specificity"])
pos <- c(cm_test_cart$byClass["Pos Pred Value"],
         cm_test_cart_cv$byClass["Pos Pred Value"],
         cm_test_svm$byClass["Pos Pred Value"],
         cm_test_svm_cv$byClass["Pos Pred Value"],
         cm_test_ada$byClass["Pos Pred Value"],
         cm_test_rf$byClass["Pos Pred Value"],
         cm_test_gbm$byClass["Pos Pred Value"],
         cm_test_gbm_cv$byClass["Pos Pred Value"],
         cm_test_xgb$byClass["Pos Pred Value"],
         cm_test_mlp$byClass["Pos Pred Value"])
tab <- data.frame(models, 
                  round(acc, 4),
                  round(sens, 4),
                  round(neg, 4),
                  round(spec, 4),
                  round(pos, 4)) %>% 
       arrange(- Accuracy) %>%
       `colnames<-`(c("MODEL", "Accuracy", "Sensitivity", "Neg Pred Val",
...
# Among the best three models, svmRadialCost + cv is the best balanced 
# between sensitivity and specificity.

# In sensitivity, the best model by far is xgbLinear.
# It's better in false negatives, which has always been the weak point
# in estimates up to now. 

###########
###########

# Ensemble solution nr 1

df <- data.frame((as.integer(pred_svm) - 2) * -1,
                 (as.integer(pred_rf) - 2) * -1,
                 (as.integer(pred_xgb) - 2) * -1,
                 (as.integer(pred_gbm_cv) - 2) * -1,
                 (as.integer(pred_adaboost) - 2) * -1)
votes <- rowSums(df)
votes <- ifelse(votes > 2, 1, 0)                 
votes <- ifelse(votes == 1, "Appreciating", "Critisizing")
mean(votes == test$sentiment)

df <- data.frame((as.integer(pred_svm) - 2) * -1,
                 (as.integer(pred_rf) - 2) * -1,
                 (as.integer(pred_xgb) - 2) * -1)
votes <- rowSums(df)
votes <- ifelse(votes > 0, 1, 0)                 
votes <- ifelse(votes == 1, "Appreciating", "Critisizing")
mean(votes == test$sentiment)
...
# Validation
# Before, the corpus was built only on the training rows (ind_train).
...
# Removing apostrophes and hyphens at the beginning of reviews,
# with repetition. 
for (i in 1:nrow(reviews)) {
  for (j in 1:12) {
...
# Removing apostrophes and hyphens at the end of reviews,
# with repetition.
for (i in 1:nrow(reviews)) {
  for (j in 1:12) {
    corpus[[i]]$content <- sub("[[:punct:]]$","", corpus[[i]]$content)
...
# become eligible after punctuation removal and stemming. 
for (i in 1:nrow(reviews)) {
  for (j in 1:length(subj_neg_multigrams)) {
    corpus[[i]]$content <- gsub(subj_neg_multigrams[j], " subjneg ", 
                                corpus[[i]]$content)
...
trControl = trainControl(method = "cv", number = 10, p = .9)
set.seed(1)
fit_train_cart_cv <- train(sentiment ~ .,
                      method = "rpart",
                      data = train,
                      tuneLength = 15, 
                      metric = "Accuracy",
...
fitted_train_cart_cv <- predict(fit_train_cart_cv)
cm_train_cart_cv <- confusionMatrix(as.factor(fitted_train_cart_cv), 
                    as.factor(train$sentiment))

# Making predictions on val.
pred_val_cart_cv <- predict(fit_train_cart_cv, newdata = val)
cm_val_cart_cv <- confusionMatrix(as.factor(pred_val_cart_cv), 
                                  as.factor(val$sentiment))

# SVM + CV
trControl = trainControl(method = "cv", number = 10, p = .9)
fit_train_svm_cv <- train(sentiment ~ .,
                     method = "svmRadialCost",
                     data = train,
                     tuneLength = 15, 
                     metric = "Accuracy",
                     trControl = trControl) 
fitted_train_svm_cv <- predict(fit_train_cart_cv)
cm_train_svm_cv <- 
  confusionMatrix(as.factor(fitted_train_svm_cv), as.factor(train$sentiment))

# Making predictions on val.
pred_val_svm_cv <- predict(fit_train_svm_cv, newdata = val)
cm_val_svm_cv <- confusionMatrix(as.factor(pred_val_svm_cv), 
                                 as.factor(val$sentiment))

# Random forest model
fit_train_rf <- train(sentiment ~ ., method = "rf", data = train) 
fitted_train_rf <- predict(fit_train_rf)
cm_train_rf <-
  confusionMatrix(as.factor(fitted_train_rf), as.factor(train$sentiment))

# Making predictions.
pred_val_rf <- predict(fit_train_rf, newdata = val)
cm_val_rf <-
  confusionMatrix(as.factor(pred_val_rf), as.factor(val$sentiment))

# xgb
fit_train_xgb <- train(sentiment ~ ., method = "xgbLinear", data = train) 
fitted_train_xgb <- predict(fit_train_xgb)
cm_train_xgb <-
  confusionMatrix(as.factor(fitted_train_xgb), as.factor(train$sentiment))

# Making predictions.
pred_val_xgb <- predict(fit_train_xgb, newdata = val)
cm_val_xgb <-
  confusionMatrix(as.factor(pred_val_xgb), as.factor(val$sentiment))

df <- data.frame((as.integer(pred_val_svm_cv) - 2) * -1,
                 (as.integer(pred_val_rf) - 2) * -1,
                 (as.integer(pred_val_xgb) - 2) * -1)
votes <- rowSums(df)
