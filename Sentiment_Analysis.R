...
write.csv(tab_av_0_1, "tab_av_0_1.csv")
tab_av_0_1 <- read.csv("tab_av_0_1.csv") %>% select(- 1)
...
description <- c("negational unigrams + polarization + rpart + tuning")
...
# CART 
# Here, raprt is trained by the train() function instead of the 
...
colname <- c("MODEL", "ACCURACY ON THE TRAINING SET")
tab_test <- data.frame(models, accs = round(accs, 4)) %>%
            arrange(- accs) %>% `colnames<-`(colname)
...
# The 3 models with extra tuning need separate treatment since 
# the number of tuned values of parameters is 15 instead of 3;  
# aggreagting them in the resamples() function would disrupt output.  
...
# Rearranging models in one table
models <- c("cart",
            "cart_tuned",
            "svm",
            "svm_tuned",
...
# This table gives a partial view of the resample accuracy distributions.
# This view already opens up more avenues to separate models on accuracy 
# merits: while accuracy medians are all above 90 % except for cart and mlp,
# accuracy minimums diverge much more. 

# The highest ranked model in resample accuracy median is svm_tuned with 93 %,
# xgb coming second with 92 %. In resample accuracy minimum, ranking is inverted
# with svm_tuned fall at 81 % and xgb remaining at 90 %.

# These distributions do not seem to be really centered around the means.
# Let's have a broader picture with a panel of graphs.
...
rm(dist)
i <- 1
l <- list(graph_1, graph_2, graph_3, graph_4, graph_5, graph_6, graph_7,
          graph_8, graph_9, graph_10)

for (i in 1:length(models)) {
  graph <- distributions %>%
    ggplot(aes("cart")) + 
    geom_histogram(bins = 7, color = "#007ba7", fill = "#9bc4e2") + 
    geom_vline(aes(xintercept = mean(cart)),
               col='magenta',size=2) +
    ggtitle("cart") +
    theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
...
# Distribution of svm_tuned
dist_svm_tuned <- fit_svm$resample$Accuracy
graph_prob_svm_tuned <- dist_svm_tuned %>% as.data.frame() %>%
  `colnames<-`("dist") %>%
  ggplot(aes(dist_svm_tuned)) + 
  geom_histogram(bins = 7, color = "#007ba7", fill = "#9bc4e2") + 
  geom_vline(aes(xintercept = mean(dist_svm_tuned)),col='magenta',size=2) +
  ggtitle("Accuracy Distribution on svm_tuned Resamples") +
  xlab("Accuracy per Resample") + ylab("Count of Resamples") +
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
        axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), 
        axis.text.x = element_text(size = 12), axis.text.y =element_text(size = 12))
graph_prob_svm_tuned

dist_rf <- fit_rf$resample$Accuracy
graph_prob_rf <- dist_rf %>% as.data.frame() %>%
  `colnames<-`("dist") %>%
  ggplot(aes(dist_rf)) + 
  geom_histogram(bins = 7, color = "#007ba7", fill = "#9bc4e2") + 
  geom_vline(aes(xintercept = mean(dist_rf)),col='magenta',size=2) +
  ggtitle("Accuracy Distribution on svm_tuned Resamples") +
  xlab("Accuracy per Resample") + ylab("Count of Resamples") +
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
        axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), 
...
l <- list(graph_prob_svm_tuned, graph_prob_rf)
marrangeGrob(l, nrow = 1, ncol = 2)

results <- resamples(list(fit_cart, 
                          fit_svm,
                          fit_adaboost,
                          fit_rf,
                          fit_gbm,
...
results_tuned <- resamples(list(fit_cart_tuned,
                                fit_svm_tuned,
                                fit_gbm_tuned))

models <- c("rpart", "rpart + tuning", "smvRadialCost", "svmRadialCost + tuning",
            "adaboost", "rf", "gbm", "gbm + tuning", "xgbLinear", "monmlp")

# €nsemble model ada / rf / xgb
df <- data.frame((as.integer(pred_xgb) - 2) * -1,
...
min(fit_svm_tuned$results$Accuracy)
min(fit_xgb$results$Accuracy)
min(fit_svm$results$Accuracy)
min(fit_rf$results$Accuracy)
min(fit_adaboost$results$Accuracy)
min(fit_cart_tuned$results$Accuracy)
min(fit_gbm_tuned$results$Accuracy)
min(fit_gbm$results$Accuracy)
min(fit_monmlp$results$Accuracy)
min(fit_cart$results$Accuracy)

str(fit_svm_tuned$results)

max(fit_cart_tuned$resample$Accuracy)

mean(fit_cart$resample$Accuracy)
mean(fit_cart$results$Accuracy)
mean(fit_cart$resample$Accuracy) == fit_cart$results$Accuracy[1]

mean(fit_cart_tuned$resample$Accuracy)
mean(fit_cart_tuned$results$Accuracy)
mean(fit_cart_tuned$resample$Accuracy) == fit_cart_tuned$results$Accuracy[1]
summary(fit_cart_tuned$resample$Accuracy)

mean(fit_rf$resample$Accuracy)
mean(fit_rf$results$Accuracy)
mean(fit_rf$resample$Accuracy) == fit_rf$results$Accuracy[2]

mean(fit_xgb$resample$Accuracy)
mean(fit_xgb$results$Accuracy)
mean(fit_xgb$resample$Accuracy) == fit_xgb$results$Accuracy[3]
summary(fit_xgb$results$Accuracy)
summary(fit_xgb$resample$Accuracy)
summary(fit_svm$resample$Accuracy)
