...
df <- data.frame(fit_cart_tuned_av0$bestTune) %>%
  `rownames<-`("CART with Tuning on cp") %>% 
  `colnames<-`("Value of cp from Final Model")
knitr::kable(df, "pandoc")
rm(df)
...
prp(fit_cart_tuned_av0$finalModel, uniform = TRUE, cex = 0.5, 
    box.palette = "auto")
...
rm(reviews_train)
rm(corpus_av2, dtm_av2, sparse_av2, sentSparse_av2)
...
rm(reviews)
rm(stopwords_negation, stopwords_with_apostrophe, 
   stopwords_without_apostrophe, extra_stopwords)
...
knitr::kable(tab, "html", align = "c") %>% 
  kable_styling(bootstrap_options = c("bordered", "condensed"), 
                full_width = F, font_size = 16) %>%
  row_spec(1:length(fits), bold = T, color = "white", background = "blue") 
...
# Let's draw graphs with the resample accuracy distribution from each model. 
...
# What's new in comparison with information delivered by predicting
# on the whole training set? 

# First, levels are lower even if means and medians are above 90 %
# for most models, with the exceptions of cart and monmlp which are 
# thereby loosing chances to be picked for validation on the validation set.

# For each model, even the maximum resampled
# accuracy is lower than the training set accuracy. The average accuracy is
# still lower. The sharpest fall can be seen for the svm and svm_tuned models.

# Expressed in means, differences between most models are more limited. 
# The svm_tuned model, which outperformed the other models on the whole
# training set, has almost been caught up by xgb in average and has  
# even been passed in median. 

# What can be seen as a serious handicap for some models is 
# a left-skewed distribution, even if it is only due to 2 resamples.
# A long left tail is problematic from a risk management
# viewpoint since some resamples show accuracy levels that are 
# substantially lower than the average and the median, even if
# these resamples are a minority. It's the case for monmlp (already out) 
# but also for the highly performing svm and svm_tuned at least in average
# and median. Thus svm and svm_tuned will also be
...
# Let's immediately order df on the basis of the minimum of resampled accuracy.
df <- df %>% arrange(- Min.)
...
rm(train, ind_train, ind_val)
rm(tab, df, l, graph, cm, colname_methods, colname_results, expressio)
...
pred_val_xgb <- predict(fits$xgb, newdata = val)
pred_val_rf <- predict(fits$rf, newdata = val)
pred_val_gbm_tuned <- predict(fits$gbm_tuned, newdata = val)
...
acc <- round(mean(votes == val$sentiment), 4)

tab <- data.frame(matrix(c(0.5000, acc), nrow = 2, ncol = 1) * 1) %>%
  `colnames<-`("Accuracy on the Validation Set") %>% 
  `rownames<-`(c("Baseline Model", 
     "Final Model: NLP - Text Mining - ML Ensemble (xgb + rf + gbm)"))
...
rm(fits)
rm(val, pred_val_gbm_tuned, pred_val_rf, pred_val_xgb, df, votes, acc)
