...
author: "[White space created before table of contents by using the author CSS selector (see file styles.css)]"
...
reviews <- reviews %>% 
  ...
  # By the way, the leading white space character in " Pos" 
...
knitr::kable(tab, "html", align = "c") %>% 
  ...
  row_spec(2, color = "white", background = "#A41034")
...
datatable(tab, rownames = FALSE, filter = "top", 
          options = list(pageLength = 10, scrollX = T,
            ...
            rowCallback = JS(
              ...
              'console.log(rowId)',
              'if(rowId >= 0) {',
                   'row.style.backgroundColor = "#9bc4e2";','}',
              '}')
            )
          )
...
# First, let's build up a data frame with only the 50 most frequent tokens from "sentSparse", i.e. the Document Term Matrix pruned by the sparsity process. 
...
graph <-  
  ...
  geom_line(col = "#08457E", size = 1) +
  geom_point(col = "#A41034", size = 4) +
...
datatable(df, rownames = FALSE, filter = "top", 
          options = list(pageLength = 4, scrollX = T,
            ...
            rowCallback = JS(
              ...
              'console.log(rowId)',
              'if(rowId >= 0) {',
                   'row.style.backgroundColor = "#b38b6d";','}',
              '}')
            )
          )
```
...
#### 2. First Scenario among False Negatives: Subjective Information Unigrams Unused

When consulting the table of false negatives above, we can pinpoint some words/tokens encompassing some subjective information that points to the right polarity.  
...
The first category is sentiment-related and so is the second category in most cases and to some degree. The three categories can also be deemed as compliance-related, expressing to some degree compliance with expectations, requirements or advertisements. 
...
Let's have a look at the final decision tree delivered by CART. 
